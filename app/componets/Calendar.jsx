...
function Calendar({ templesData, TodayDate }) {
  ...
  return (
    <div className='flex flex-col'>
      <div className='m-3 p-1 text-center rounded-md text-white bg-red-600 flex flex-wrap'>
        {months.map((month, index) => (
          <div 
            key={index} 
            className={`ml-3 cursor-pointer p-1 rounded ${selectedMonth === month ? 'bg-red-600 text-white' : ''}`}
      ...
      {selectedMonth && (
        <div className='m-3 p-2 bg-red-100 text-center'>
          ...
          <div className='flex flex-wrap justify-center'>
            {filteredTemples.length > 0 ? (
              filteredTemples.map((item, index) => (
                <div key={index} className='text-zinc-800 cursor-default my-5 bg-gradient-to-tr from-red-200 to-red-400 rounded-md border-4 border-red-200 p-5 flex flex-wrap items-center justify-center lg:flex-row mr-5 ml-5 w-56 md:w-96'>
